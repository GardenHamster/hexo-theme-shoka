...
hexo.extend.helper.register('_background', function (item) {
  const { statics, js, background_list } = hexo.theme.config;
  if (!background_list || background_list.length === 0) return null;
  const randomIndex = parseInt(Math.random() * background_list.length)
  return background_list[randomIndex];
})
